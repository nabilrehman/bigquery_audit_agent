...
from google.oauth2 import service_account  # noqa: F401 (support if env provides SA)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=90,
        help="Lookback window in days for job history (default: 90)",
    )
    parser.add_argument(
        "--outfile",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Max jobs to pull per location for the CSV (default: 1000)",
    )
    return parser.parse_args()


def _jobs_query_sql(days: int, limit: int) -> str:
    return (
    ...
    ).format(days=days, limit=limit, schema="{schema}")
...
def _pick_schema_for_location(location: str) -> str:
    ...
    # Fallback: allow explicit region datasets if user passes something else, though this script targets multi-regions
...
def _fetch_jobs(client: bigquery.Client, location: str, days: int, limit: int) -> List[JobStat]:
    ...
    sql = _jobs_query_sql(days=days, limit=limit).format(schema=schema)
...
def _most_expensive(jobs: List[JobStat]) -> Optional[JobStat]:
    if not jobs:
        return None
    # Max by billed bytes, tie-breaker total_slot_ms
...
def main() -> int:
    ...
    # Authentication: relies on ADC (gcloud auth application-default login)
    # Alternatively, set GOOGLE_APPLICATION_CREDENTIALS to a JSON SA key file.
